import { Home, Search, PlusSquare, Heart, User, ShoppingBag, MessageSquare, Clapperboard } from 'lucide-react';
...
export default function Navbar() {
  ...
  return (
    <>
      ...
      <nav className="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 p-4 flex justify-between items-center sm:top-0 sm:bottom-auto sm:border-t-0 sm:border-r sm:w-64 sm:h-screen sm:flex-col sm:items-start sm:p-6 z-10">
        ...
        <div className="flex justify-between w-full sm:flex-col sm:gap-6">
          ...
          <NavItem icon={<Clapperboard className="w-6 h-6" />} label="Reels" to="/reels" />
          <NavItem icon={<PlusSquare className="w-6 h-6" />} label="Create" to="/create" />
          <NavItem icon={<MessageSquare className="w-6 h-6" />} label="Messages" to="/messages" />
